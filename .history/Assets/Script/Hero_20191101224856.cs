  ...
  GameObject Object;
    ...
    private void OnTriggerEnter2D(Collider2D other) {
      if(other.gameObject.tag == "enemyball"){
        FindObjectOfType<HP>().Damage(5);
        rigidbody.velocity = new Vector2((other.transform.position.x - rigidbody.position.x) * 100, (other.transform.position.y - rigidbody.position.y)*100);

      }
      if(other.gameObject.tag == "BossBall"){
        FindObjectOfType<HP>().Damage(8);
      }
    }
    private void OnCollisionStay2D(Collision2D other) {
      if(other.gameObject.tag == "enemy"){
        /*vect = other.relativeVelocity;
        //rigidbody.AddForce(other.relativeVelocity * 500);
        while(i <= 10){
          rigidbody.AddForce(vect * 50);
          i++;
        }
        i = 1;*/
      }
      if(other.gameObject.tag == "Boss"){
        rigidbody.AddForce(other.relativeVelocity * 1000);
      }
    }
}
